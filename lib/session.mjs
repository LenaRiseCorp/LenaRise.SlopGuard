...
export function emptySession(sessionId) {
  return {
    ...
    testRunAt: null,
    commitAt: null,
    codeWritesSinceVerify: 0,  // son testten beri yazılan kod dosyası sayısı
    stopBlocks: {},            // ihlal parmak izi → kaç kez bloklandı (AGT-08 döngü koruması)
...
export function recordWrite(state, filePath, { added = 0, removed = 0, isCode = false } = {}) {
...
export function recordTestRun(state) {
  ...
  state.codeWritesSinceVerify = 0;   // doğrulama borcu kapandı
}
...
/**
 * Stop kapısının aynı gerekçeyle kaç kez bloklandığını sayar.
 *
 * Parmak izi ihlal kümesinden üretilir: küme değiştiyse ilerleme var demektir
 * ve sayaç sıfırdan başlar. Değişmediyse artar — kendi AGT-08 kuralımız
 * (kısır döngü) sonsuz bloklamayı yasaklıyor, o yüzden bir tavan gerekli.
 */
