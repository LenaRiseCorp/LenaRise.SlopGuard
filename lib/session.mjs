...
export function emptySession(sessionId) {
  return {
    ...
    interfaceWritesSinceReport: 0,  // interface files written since the last delivery report
    stopBlocks: {},            // violation fingerprint → block count (AGENT-08 loop guard)
    testRunAt: null,
    deliveryReportAt: null,
    commitAt: null,
 ...
 */
export function recordWrite(state, filePath, { added = 0, removed = 0, isCode = false, isInterface = false, inRepo = true } = {}) {
  if (isCode) state.codeWritesSinceVerify += 1;
  if (isInterface) state.interfaceWritesSinceReport = (state.interfaceWritesSinceReport ?? 0) + 1;
...
/**
 * A delivery report was produced by running the gate, not by claiming one.
 * Only a clean run reaches here — scripts/deliver.mjs records nothing when it
 * finds something, so the debt is settled by a fix, never by a re-run.
 */
export function recordDeliveryReport(state) {
  state.deliveryReportAt = Date.now();
  state.interfaceWritesSinceReport = 0;
}
