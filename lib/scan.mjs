...
/**
 * Heredoc gövdelerini aynı uzunlukta boşlukla değiştirir; satır sonları korunur.
 *
 * Neden: `cat > x.md <<'EOF' … EOF` arasındaki metin kabuk kodu değil, veridir.
 * Komut kapsamı komutu tarar; gövdede geçen yıkıcı bir ifade o metinde
 * *anılıyor* olabilir, çalıştırılıyor değil.
 *
 * Kapsam kaybı yok: heredoc bir dosyaya yazılıyorsa o dosya post-bash içerik
 * taramasından geçer. Komut kapsamı komutu tarar, içerik kapsamı içeriği —
 * gövdedeki gerçek tehlike doğru katmanda yakalanır.
 *
 * Bu ayrım aracın kendi geliştirilmesinde ortaya çıktı: yıkıcı bir komuttan
 * söz eden commit mesajı, yıkıcı komut sanılıp engellendi.
 */
...
/** Kabuk komutunu tarar (çalıştırılmadan önce). Heredoc gövdesi komut değil, veridir. */
