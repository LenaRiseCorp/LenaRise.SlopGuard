...
/** "3 gün önce" gibi okunur yaş. Durum çubuğu, koç ve /slop-doctor kullanır. */
...
/**
 * Durum ölçümleri — durum çubuğu ile sohbet satırının ortak kaynağı.
 *
 * İkisi aynı bilgiyi gösteriyor; iki ayrı yerde biçimlendirmek ilk değişiklikte
 * ayrışırdı (KOD-01). Fark yalnızca girdide: satır sayıları statusLine yüküyle
 * geliyor, hook'lara gelmiyor — o yüzden opsiyonel.
 */
export function statusMetrics(state, config, { added = null, removed = null } = {}) {
  const parts = [
    config.mode === 'explore' ? 'keşif' : 'sert',
    `${state.blocked} engellendi`,
    `tur ${state.turns}/${config.thresholds.contextTurns}`,
  ];
  if (added !== null) parts.push(`+${added}/-${removed ?? 0}`);
  parts.push(state.testRunAt
    ? `test ${formatAge(Math.round((Date.now() - state.testRunAt) / 1000))}`
    : 'test yok');
