...
export const DEFAULT_CONFIG = Object.freeze({
  enabled: true,               // kullanıcı bilerek kapattıysa false — çubuk 'kapalı' gösterir
  ...
  ui: Object.freeze({
    ...
    chatStatus: 0,             // 0 = kapalı; N = her N turda bir sohbete durum satırı
  }),
});
...
export function loadConfig({ repoRoot, sessionMode } = {}) {
  ...
  if (!file.ok) {
    ...
    for (const [k, v] of Object.entries(u.ui ?? {})) {
      if (k === 'chatStatus') {
        // Sayısal alan; enum kontrolü onu sessizce geçirirdi.
        if (Number.isInteger(v) && v >= 0) config.ui.chatStatus = v;
        else problems.push(`config.json → ui.chatStatus: 0 veya pozitif tam sayı bekleniyordu ("${v}")`);
        continue;
      }
