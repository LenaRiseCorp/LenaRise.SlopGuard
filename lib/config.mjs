...
import { TEST_PATH_PATTERNS, SCOPES } from './patterns.mjs';
...
export function actionFor(finding, config) {
  ...
  // A leaked-secret finding inside a test is warned about, not blocked. A test
  // that verifies credential handling has to contain credential-shaped strings;
  // that is the test doing its job. Measured across 23 repositories: 100 of the
  // SEC-03 findings were invented credential strings sitting in .test.mjs files,
  // and every one of them would have refused a commit.
  //
  // Warned, not silenced: a real key can be pasted into a test as easily as
  // anywhere else, and the finding stays on screen. Only the refusal is dropped.
  if (finding.category === 'SEC' && isTestFile(finding.target)) return 'warn';
  return config.mode === 'explore' ? 'warn' : 'block';
}

/**
 * A test path, by the same list the scanner uses.
 *
 * Duplicated here rather than imported from scan.mjs, which imports this module:
 * both read TEST_PATH_PATTERNS from the pattern registry, so there is still one
 * source for what counts as a test.
 */
function isTestFile(filePath) {
  const p = String(filePath || '').replace(/\\/g, '/');
  return p !== '' && TEST_PATH_PATTERNS.some((re) => re.test(p));
}
