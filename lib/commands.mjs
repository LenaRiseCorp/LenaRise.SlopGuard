...
/**
 * `npm run <name>` says nothing about what the script does. Recognising the name
 * would guess in the dangerous direction: a repository whose `verify` only lints
 * would collect a test stamp for tests that never ran, which is the failure this
 * file exists to prevent. So the script body is read and classified by the same
 * patterns, one level at a time.
 */
const RUN_SCRIPT = /^(?:npm|pnpm|yarn|bun)\s+run\s+([^\s&|;]+)/;
const MAX_SCRIPT_DEPTH = 3;

/** The body of one package.json script, or null when there is nothing to read. */
function scriptBody(cwd, name) {
  let raw;
  try {
    raw = readFileSync(join(cwd, 'package.json'), 'utf8');
  } catch (error) {
    // Outside a Node project there is simply no package.json; that is the normal
    // case, not a fault. Anything else is reported rather than swallowed.
    if (error.code !== 'ENOENT') fail('commands', `package.json could not be read (${cwd}) — ${error.message}`);
    return null;
  }
  try {
    ...
    fail('commands', `package.json is not valid JSON (${cwd}) — ${error.message}`);
    return null;
  }
}

function segmentRunsTests(command, cwd, seen, depth) {
  return commandSegments(command).some((seg) => {
    if (TEST_COMMAND_PATTERNS.some((re) => re.test(seg))) return true;
    if (!cwd || depth >= MAX_SCRIPT_DEPTH) return false;

    const name = RUN_SCRIPT.exec(seg)?.[1];
    // A script that reaches itself would otherwise recurse to the depth cap;
    // stopping at the repeat keeps the answer the same and the work bounded.
    if (!name || seen.has(name)) return false;
    seen.add(name);

    const body = scriptBody(cwd, name);
    return body !== null && segmentRunsTests(body, cwd, seen, depth + 1);
  });
}

/**
 * Did this command run tests?
 *
 * @param {string} command
 * @param {{cwd?: string}} [options]  cwd enables package.json script resolution.
 *   Without it only the direct patterns answer, which is the conservative side:
 *   no evidence produces no stamp.
 */
export function isTestCommand(command, { cwd } = {}) {
  return segmentRunsTests(command, cwd, new Set(), 0);
}
...
export function parseInstall(command) {
  // Segments rather than the raw string. This function used to split on `&&`,
  // `||`, `;` and `|` by itself, which left it without the two things
  // commandSegments had already learned: heredoc bodies are data, and a newline
  // separates commands. The cost was not theoretical — a heredoc writing sample
  // text that merely contained an install line was parsed as an install, the
  // tool call was denied, and the "package names" reported back were fragments
  // of the surrounding code.
  //
  // scanCommand already strips heredocs before matching command patterns, so
  // this also ends an inconsistency: an install line inside a heredoc was
  // invisible to the pattern layer and visible here.
  const segments = commandSegments(command);
  const manager = MANAGERS.find((m) => segments.some((seg) => m.re.test(seg)));
  if (!manager) return null;

  // Take only the matching segment, so words earlier in a chain such as
  // `cd x && npm install foo` are not mistaken for package names. The segment
  // is present by construction — the manager was found by scanning these.
  const segment = segments.find((seg) => manager.re.test(seg));
...
export function writeTargets(command) {
  // Stripped once, up front. The redirection scan below and inlineCodeTargets
  // both read the whole command rather than segments, and both were reading
  // heredoc bodies with it: a document showing `printf x > .env` as an example
  // was reported as writing to .env, and pre-bash denies a write to a protected
  // path. The opener line survives stripping, so `cat > file <<'EOF'` — the
  // common real case — is still seen.
