 ...
 *
 * Satır sonu da ayırıcıdır. İlk sürüm yalnızca `&&`, `||`, `;` ve `|` üzerinden
 * bölüyordu; çok satırlı bir bloğun ikinci satırındaki `git commit` ya da
 * `npm test` hiç tanınmıyordu. Sonucu sessiz ve pahalıydı: commit'ler oturum
 * defterine işlenmiyor, doğrulama damgası atılmıyor, Stop kapısı da bu yüzden
 * yapılmış işi yapılmamış sayıyordu. Aracın kendi geliştirilmesinde ortaya çıktı.
 *
 * Heredoc gövdeleri önce ayıklanır: commit mesajının içindeki bir satır komut
 * değildir. Aynı ayrım scanCommand'de de var — gövde veridir, kabuk kodu değil.
 */
