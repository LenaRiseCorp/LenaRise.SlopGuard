...
export const CATEGORIES = {
  ...
  OYN: { name: 'Oyun geliştirme',    layer: 'makine', enforcement: 'alan bazlı' },
};
...
export const TAXONOMY = [
  ...
  ['OYN-02', 'Her karede sahne araması'],
  ['OYN-03', 'Fiziğin kare döngüsünde yürütülmesi'],
  ['OYN-04', 'Sıcak yolda bellek tahsisi'],
  ['OYN-05', 'Her karede günlük yazımı'],
  ['OYN-06', 'İstemcide tutulan ekonomi ve ilerleme'],
  ['OYN-07', 'Kırılgan sahne ağacı yolu'],
  ['OYN-08', 'Motor üretimi dosyanın elle düzenlenmesi'],
 ...
 */
export const NEW_IDS = ['SUR-08', ...Array.from({ length: 8 }, (_, i) => `OYN-0${i + 1}`)];

/**
 * OYN kategorisi kaynak taksonomide yok; bu proje ekledi.
 *
 * Neden ayrı kategori: oyun desenleri motor API adlarına dayandığı için başka
 * projelerde zaten tetiklenmez, ama kategori olması onları tek satırla
 * kapatılabilir kılıyor (`disabled: ["OYN"]`). Mevcut kategorilere dağıtmak
 * hem keşfedilmesini zorlaştırır hem toplu kapatmayı imkânsız kılardı.
 *
 * Karşılıkları tek tek de var: OYN-06 bir GUV meselesi, OYN-01 ve OYN-03 birer
 * MTK hatası, OYN-02/04/05 birer KOD sorunu. Kategori bunları reddetmiyor,
 * alanı adlandırıyor — bir Unity projesinde bu hataların hepsi aynı yerden gelir.
 */
...
export const PROTECTED_PATH_PATTERNS = [
  ...
  // Oyun motorlarının ürettiği dosyalar. Bunları elle düzenlemek sessiz ve
  // pahalı bozulmalar üretir: .meta dosyasındaki GUID değişirse Unity sahnedeki
  // her referansı kaybeder, .uasset ve .umap ikili biçimdir ve metin olarak
  // yazılınca geri dönüşsüz bozulur. Motor bunları kendisi üretir.
  { re: /\.meta$/,                            why: 'OYN-08 · Unity varlık kimliği (GUID) — elle düzenlenirse sahne referansları kopar' },
  { re: /(^|\/)(?:Library|Temp|Obj|Logs|UserSettings)\//i, why: 'OYN-08 · Unity üretim dizini' },
  { re: /(^|\/)ProjectSettings\/ProjectVersion\.txt$/, why: 'OYN-08 · Unity sürüm damgası' },
  { re: /(^|\/)\.godot\//,                    why: 'OYN-08 · Godot üretim dizini' },
  { re: /\.(?:uasset|umap|unity|prefab|asset|tscn|tres|blend|fbx)$/i, why: 'OYN-08 · motor varlık dosyası — ikili ya da üretilmiş biçim' },
  { re: /(^|\/)(?:Binaries|Intermediate|Saved|DerivedDataCache)\//, why: 'OYN-08 · Unreal üretim dizini' },
];
...
export const PATTERNS = [
  ...
  // ── OYN — oyun geliştirme ────────────────────────────────────────────────
  //
  // Bu desenler motor API adlarına dayanır, dolayısıyla oyun olmayan
  // projelerde kendiliğinden sessiz kalır. Sertlikleri bilerek `warn`:
  // sıcak yol tespiti sezgiseldir ve yeni bir alanda blokla başlamak,
  // aracın ilk izlenimini yanlış pozitifle kurmak olurdu. Tek istisna
  // OYN-06, çünkü o bir güvenlik meselesi.
  //
  // `UPDATE` gövde kalıbı sınırlı ve temperlenmiş: sonraki metot
  // bildirimine kadar bakar, 500 karakteri aşmaz. Ölçüldü — 400 satırlık
  // eşleşmeyen gövdede 0,1 ms.
  {
    key: 'oyn-01-framerate-bagimli-hareket', id: 'OYN-01', scope: 'code', severity: 'warn',
    match: /\btransform\.Translate\s*\((?![^)]*\bTime\.)[^)]*\)|\btransform\.position\s*\+=\s*(?![^;\n]*\bTime\.)[^;\n]*[*][^;\n]*;/g,
    detects: 'Hareket kare hızına bağlı — Time.deltaTime ile ölçeklenmemiş.',
    fix: 'Hızı Time.deltaTime ile çarp; yoksa oyun 30 ve 144 fps’de farklı hızda çalışır.',
  },
  {
    key: 'oyn-02-her-karede-sahne-aramasi', id: 'OYN-02', scope: 'code', severity: 'warn',
    ...
    detects: 'Kare döngüsünde sahne araması ya da bileşen çözümlemesi.',
    fix: 'Referansı Awake içinde bir kez çöz ve alanda sakla, ya da [SerializeField] ile bağla.',
  },
  {
    key: 'oyn-03-fizik-update-icinde', id: 'OYN-03', scope: 'code', severity: 'warn',
    ...
    detects: 'Fizik çağrısı Update içinde — fizik adımıyla senkron değil.',
    fix: 'Rigidbody işlemlerini FixedUpdate içine taşı.',
  },
  {
    key: 'oyn-04-sicak-yolda-tahsis', id: 'OYN-04', scope: 'code', severity: 'warn',
    ...
    detects: 'Kare döngüsünde LINQ — her karede çöp üretir, takılma yaratır.',
    fix: 'Sonucu önceden hesapla ya da döngüyü elle yaz; sıcak yolda tahsis yapma.',
  },
  {
    key: 'oyn-05-her-karede-gunluk', id: 'OYN-05', scope: 'code', severity: 'warn',
    ...
    detects: 'Her karede günlük yazımı — editörde kare süresini gözle görülür düşürür.',
    fix: 'Kaldır, ya da bir koşula veya #if UNITY_EDITOR bloğuna bağla.',
  },
  {
    key: 'oyn-06-istemcide-ekonomi', id: 'OYN-06', scope: 'code', severity: 'block',
    match: /\bPlayerPrefs\.Set(?:Int|Float|String)\s*\(\s*["'][^"']*(?:coin|gem|gold|money|currency|credit|score|level|xp|premium|purchase|unlock|para|altin|puan|skor)/gi,
    detects: 'Ekonomi ya da ilerleme değeri istemcide saklanıyor (GUV-04).',
    fix: 'PlayerPrefs düz metindir ve oyuncu tarafından düzenlenebilir. Değeri sunucuda tut ya da imzala.',
  },
  {
    key: 'oyn-07-kirilgan-dugum-yolu', id: 'OYN-07', scope: 'code', severity: 'warn',
    match: /\b(?:get_node|GetNode(?:<[^>]*>)?)\s*\(\s*["'][^"']*\.\.\//g,
    detects: 'Göreli sahne ağacı yolu — düğüm taşınınca sessizce kopar.',
    fix: '@onready ile dışa aktarılmış NodePath kullan ya da sinyalle bağla.',
  },
  {
