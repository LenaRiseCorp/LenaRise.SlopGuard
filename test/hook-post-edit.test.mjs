...
test('kirli dosya block döner, kategori ve satır taşır', () => {
  ...
  assert.match(r.json.reason, /satır 3/);
  assert.match(r.json.reason, /dirty\.js/);
});

test('temiz dosya sessiz geçer — çıktı yok, hata yok', () => {
  const f = ws.file('clean.js', 'try {\n  risky();\n} catch (e) {\n  logger.warn(e);\n  throw e;\n}\n');
  const r = run(f, { sessionId: 'temiz' });
  assert.equal(r.code, 0);
  assert.equal(r.stdout, '', 'temiz tarama sessizdir');
  assert.equal(r.stderr, '');
});

test('ihlal oturum defterine yazılır — stop kapısının dayanağı', () => {
  ...
  assert.equal(state.violations[f][0].id, 'KOD-05');
  assert.equal(state.blocked, 1);
});

test('düzeltilen dosya defterden düşer', () => {
  ...
  assert.deepEqual(state.violations, {}, 'düzeltme defteri temizlemeli');
});

test('kalp atışı her tetiklenmede damgalanır', () => {
...
test('devre dışı bırakılan desen bloklamaz', () => {
  ws.config({ disabled: ['KOD-05'] });
  const r = run(ws.file('disabled.js', 'try{}catch(e){}\n'), { sessionId: 'kapali' });
  assert.equal(r.stdout, '');
  ws.config({});
});

test('cleanScans summary kipinde temiz tarama bildirilir', () => {
  ws.config({ ui: { cleanScans: 'summary' } });
  const r = run(ws.file('quiet.js', 'const a = 1;\n'), { sessionId: 'ozet' });
  assert.match(r.json.systemMessage, /1 dosya tarandı · temiz/);
  ws.config({});
});

test('taranmayan uzantı sessiz geçer ama satır sayacı işler', () => {
...
test('bozuk stdin bloklamaz ama sessiz de kalmaz', () => {
...
test('silinmiş dosya için yedek içerik kullanılır', () => {
...
test('structuredPatch satır sayımı: eklenen ve silinen ayrı sayılır', () => {
